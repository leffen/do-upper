...
func (p *pingResponse) asString() string {
	return fmt.Sprintf("%06d %-30.30s Total: %v First byte after: %v  DNS: %v TLS Handshake:%v Connect: %v \n", p.pingNum, p.url, p.totalDur, p.dur, p.dnsDuration, p.tlsDuration, p.connectTime)
}
